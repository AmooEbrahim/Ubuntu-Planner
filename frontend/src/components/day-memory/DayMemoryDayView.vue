<template>
  <div class="space-y-4">
    <div v-if="track === 'ai' && readonly" class="flex items-start gap-3 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
      <svg class="h-5 w-5 flex-shrink-0 text-amber-500 mt-0.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m0 3.75h.008M21.75 12a9.75 9.75 0 1 1-19.5 0 9.75 9.75 0 0 1 19.5 0Z"/></svg>
      <div class="flex-1">
        <p class="font-medium">AI track is read-only.</p>
        <p class="text-amber-800 mt-1">The AI writes here on its own. To edit by hand, enable the override below.</p>
      </div>
      <button
        type="button"
        class="rounded-md bg-amber-600 text-white text-xs font-medium px-3 py-1.5 hover:bg-amber-700"
...
const SECTIONS = [
  {
    key: 'intentions',
    title: 'How do you want to go in this day?',
    hint: 'Set a clear intention before the day starts.',
    placeholder: 'I want to focus on…',
  },
  {
    key: 'completed',
    title: 'What did you complete today?',
    hint: 'Concrete things that got done.',
    placeholder: '— Shipped phase 1\n— Reviewed plan\n…',
  },
  {
    key: 'reflection',
    title: 'How did it actually happen?',
    hint: 'Honest summary, not a to-do list.',
    placeholder: 'The morning went well, then…',
  },
  {
    key: 'lessons',
    title: 'What did you learn from this day?',
    hint: 'Insight you want to remember.',
    placeholder: 'I noticed that…',
  },
  {
    key: 'gratitude',
    title: 'Thanksgiving',
    hint: 'Things, people, or moments you are grateful for.',
    placeholder: 'I am grateful for…',
  },
  {
    key: 'free_notes',
    title: 'Anything else',
    hint: 'Free-form notes for the day.',
    placeholder: '',
  },
]
